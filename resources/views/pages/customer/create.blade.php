    ...
    <div class="card xl:col-span-2">
        <div class="card-body flex flex-col p-6">
            <header class="flex mb-5 items-center border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
                <div class="flex-1">
                    <div class="card-title text-slate-900 dark:text-white">Form Add Customer</div>
                </div>
            </header>
            <div class="card-text h-full">
                <form action="{{ route('master-customer.store') }}" class="space-y-4" id="multipleValidation" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="grid md:grid-cols-2 gap-6">
                        <div class="input-area">
                            <label for="nama_customer" class="form-label">Nama Customer</label>
                            <input id="nama_customer" type="text" name="nama_customer" value="{{ old('nama_customer') }}" class="form-control" placeholder="Nama Customer" required>
                        ...
                        <div class="input-area">
                            ...
                            <input id="nomor_customer" type="text" name="nomor_customer" value="{{ old('nomor_customer') }}" class="form-control" placeholder="Nomor Customer" required>
                        ...
                        <div class="input-area">
                            ...
                            <input id="tanggal_lahir" type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir') }}" class="form-control" required>
                            @error('tanggal_lahir')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div class="input-area">
                            <label for="no_hp" class="form-label">No HP</label>
                            <input id="no_hp" type="text" name="no_hp" value="{{ old('no_hp') }}" class="form-control" placeholder="Nomor HP" required>
                        ...
                        <div class="input-area">
                            ...
                            <textarea id="alamat" name="alamat" class="form-control" placeholder="Alamat Lengkap">{{ old('alamat') }}</textarea>
                            @error('alamat')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div class="input-area">
                            <label for="nik" class="form-label">NIK</label>
                            <input id="nik" type="text" name="nik" value="{{ old('nik') }}" class="form-control" placeholder="Nomor NIK">
                            @error('nik')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div class="flex items-center space-x-2">
                            <label class="relative inline-flex h-6 w-[46px] items-center rounded-full transition-all duration-150 cursor-pointer">
                                <input type="hidden" name="status" value="0">
                                <input type="checkbox" name="status" class="sr-only peer" value="1" {{ old('status') == 1 ? 'checked' : '' }}>
