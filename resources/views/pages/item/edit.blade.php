    ...
    <div class="card xl:col-span-2">
        <div class="card-body flex flex-col p-6">
            <header class="flex mb-5 items-center border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
                <div class="flex-1">
                    <div class="card-title text-slate-900 dark:text-white">Form Edit Item</div>
                </div>
            </header>
            <div class="card-text h-full">
                <form action="{{ route('item.update', $item->id) }}" class="space-y-4" id="multipleValidation" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="grid md:grid-cols-2 gap-6">
                        <div class="input-area">
                            <label for="code" class="form-label">Kode Item</label>
                            <input id="code" type="text" name="code" value="{{ old('code', $item->code ?? '') }}" class="form-control" placeholder="Kode Item" required>
                            @error('code')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div class="input-area">
                            <label for="nama" class="form-label">Nama Item</label>
                            <input id="nama" type="text" name="nama" value="{{ old('nama', $item->nama ?? '') }}" class="form-control" placeholder="Nama Item" required>
                            @error('nama')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="kategori" class="form-label">Kategori</label>
                            <select name="kategori" id="kategori" class="form-control w-full mt-2 py-2" required>
                                <option value="gas" {{ old('kategori', $item->kategori ?? '') == 'gas' ? 'selected' : '' }}>Gas</option>
                                <option value="cair" {{ old('kategori', $item->kateori ?? '') == 'Cair' ? 'selected' : '' }}>Cair</option>
                            </select>
                            @error('jenis_kelamin')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div class="input-area">
                            <label for="harga" class="form-label">Harga</label>
                            <input id="harga" type="number" name="harga" value="{{ old('harga', number_format($item->harga, 0, '', '') ?? '') }}" class="form-control" placeholder="0" required>
                            @error('harga')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div class="input-area">
                            <label for="stok" class="form-label">Stok</label>
                            <input id="stok" type="number" name="stok" value="{{ old('stok', number_format($item->stok, 0, '', '') ?? '') }}" class="form-control" placeholder="0" required>
                            @error('stok')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div class="input-area">
                            <label for="satuan" class="form-label">Satuan</label>
                            <input id="satuan" type="text" name="satuan" value="{{ old('satuan', $item->satuan ?? '') }}" class="form-control" placeholder="Satuan" required>
